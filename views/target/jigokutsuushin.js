...
function playopening(){let video=getOrCreate("video","video");new modify(video).setScale("480px","270px").centralize();new append(video).tag("source",new Map([["src","/assets/jigokutsuushin/video.mp4"],["type","video/mp4"]]));let audio=getOrCreate("audio","audio");new modify(audio);new append(audio).tag("source",new Map([["src","/assets/jigokutsuushin/audio.wav"],["type","audio/wav"]]));new append(neko).child(video).child(audio);video.play();audio.play();video.onended=session1;audio.onended=()=>{remove(audio);};}
