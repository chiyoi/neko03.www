...
var timeout;function playopening(){let video=getOrCreate("video","video");new modify(video).setScale("480px","270px").setAttr("autoplay","").setAttr("muted","").centralize();new append(video).tag("source",new Map([["src","/assets/jigokutsuushin/video.mp4"],["type","video/mp4"]]));let audio=getOrCreate("audio","audio");new modify(audio).setAttr("autoplay","");new append(audio).tag("source",new Map([["src","/assets/jigokutsuushin/audio.wav"],["type","audio/wav"]]));new append(neko).child(video).child(audio);video.onended=session1;timeout=window.setTimeout(session1,5000);audio.onended=()=>{remove(audio);};}
